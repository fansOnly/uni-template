...
export function isMobileType(v) {
 ...
 * 至少包含字母、数字、符号中的2种
 */
export function isPassType(v) {
  return /^(?![-`=[\];',.~!@#$%^&*()_+|{}:"?]+$)(?![0-9]+$)(?![A-Za-z]+$)[0-9a-zA-Z-`=[\];',.~!@#$%^&*()_+|{}:"?]{8,16}$/.test(String(v))
}

export function isUrlType(v) {
...
export function isIdCardType(v) {
 ...
 * 5-50个字符，不能全数字
 */
export function isAddressType(v) {
 ...
 * 银行卡规则
 */
export function isLuhnType(bankNo) {
