<template>
  <view class="et-form-item" :style="customStyle">
    <slot name="label">
      <view v-if="label" class="et-form-item__label" :style="style">
        <text v-if="isRequired" class="is-required">*</text>
        <text>{{ label }}</text>
      </view>
    </slot>
    <view class="et-form-item__input">
      <slot />
      <view class="et-form-item__error" :class="{'is-active': errorText}">{{ errorText }}</view>
    </view>
  </view>
</template>
