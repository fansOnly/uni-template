  ...
  export default {
    ...
    computed: {
      ...
      MINUTES({ type, timeRange, dateRange, hourValue, yearValue, monthValue, dayValue, columnName }) {
        ...
        // 年 月 日 时变化时 校正分的值
        if (['year', 'month', 'day', 'hour'].includes(columnName)) {
          const [min, max] = [data[0], data[data.length - 1]]
          if (this.minuteValue > max.value) {
            this.minuteValue = max.value
          } else if (this.minuteValue < min.value) {
            this.minuteValue = min.value
          }
         }
        return data
      },
