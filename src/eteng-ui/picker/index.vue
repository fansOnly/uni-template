<template>
  <et-popup :visible="visible" round position="bottom" :max-height="rowHeight * rows" border :close-on-click-overlay="closeOnClickOverlay" body-style="padding-bottom: 32rpx;" @click-overlay="clickOverlay" @close="close" @after-leave="show = false">
    ...
    <view class="et-picker-body" :style="{ 'height': rowHeight * rows + 'px' }" @touchmove.stop="noop">
      <et-picker-column v-for="(item, index) in options" :key="index" class="et-picker-col" :options="item" :current="activeIndexes[index]" :rows="rows" :row-height="rowHeight" :style="{'max-width': 'calc(100% / '+options.length+');'}" @change="onChange($event, index)"></et-picker-column>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      maskStyled({ rowHeight, rows, addUnit }) {
        // note: 遮罩效果
        let style = ''
