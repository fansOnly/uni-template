...
<script>
  ...
  export default {
    ...
    async mounted() {
      ...
      await this.resolveStyle()
    },
    methods: {
      updateMenu() {
        this.menus = this.getDropdownItems().map((child, index) => {
      ...
      toggleItem(active = -1) {
        this.getDropdownItems().map((child, index) => {
          if (active === index) {
            child.toggle()
      ...
      close(duration = 60) {
        this.getDropdownItems().map(child => {
          child.toggle(false, duration)
        })
      },
      getDropdownItems() {
        const children = getChildrenCompatibleH5(this)
        return children.filter(child => child['useDropdownItem'])
      },
