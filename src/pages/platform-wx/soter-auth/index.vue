  ...
  methods: {
    ...
    async checkIsSoterEnrolled() {
      if (this.isSupportFingerPrint) {
        const res = await wxCheckIsSoterEnrolledInDevice({ checkAuthMode: 'fingerPrint' })
        this.isFingerPrintEnrolled = res.isEnrolled
      } else if (this.isSupportFacial) {
        const res = await wxCheckIsSoterEnrolledInDevice({ checkAuthMode: 'facial' })
