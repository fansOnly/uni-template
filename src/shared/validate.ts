...
export const toTypeString = (val: any) => objectToString.call(val)

export const toType = (val: any) => {
...
export const isMobileType = (v: unknown): boolean => {
 ...
 * 至少包含字母、数字、符号中的2种
 */
export const isPassType = (v: unknown): boolean => {
  return /^(?![-`=[\];',.~!@#$%^&*()_+|{}:"?]+$)(?![0-9]+$)(?![A-Za-z]+$)[0-9a-zA-Z-`=[\];',.~!@#$%^&*()_+|{}:"?]{8,16}$/.test(
    String(v)
  )
}

export const isUrlType = (v: unknown): boolean => {
...
export const isIdCardType = (v: unknown): boolean => {
 ...
 */
export function isWechatIdType(v) {
  return /^[a-zA-Z][a-zA-Z0-9_-]{5,19}$/.test(String(v))
}

export function isEmailType(v) {
 ...
 * 5-50个字符，不能全数字
 */
export const isAddressType = (v: unknown): boolean => {
 ...
 * 2-20个字符
 */
export const isNameType = (v: unknown): boolean => {
 ...
 * 银行卡规则
 */
export const checkLuhn = (bankNo: string): boolean => {
  if (!bankNo) return false
  const lastNum: string = bankNo.substring(bankNo.length - 1, 1) //取出最后一位（与luhn进行比较）
  const first15Num: string = bankNo.substring(0, bankNo.length - 1) //前15或18位
