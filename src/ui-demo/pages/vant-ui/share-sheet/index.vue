<template>
  <view class="page-wrapper">
    <demo-block card title="基础用法">
      ...
      <van-share-sheet :show="show.basic" title="立即分享给好友" :options="options" @close="onClose"
        @select="onSelect" />
    </demo-block>

    <demo-block card title="展示多行选项">
      <van-cell is-link title="显示分享面板" data-type="multiLine" @click="onShowShareSheet" />
      <van-share-sheet :show="show.multiLine" title="立即分享给好友" :options="multiLineOptions" @close="onClose"
        @select="onSelect" />
    </demo-block>

    <demo-block card title="自定义图标">
      <van-cell is-link title="显示分享面板" data-type="customIcon" @click="onShowShareSheet" />
      <van-share-sheet :show="show.customIcon" :options="customIconOptions" @close="onClose"
        @select="onSelect" />
    </demo-block>

    <demo-block card title="展示描述信息">
      <van-cell is-link title="显示分享面板" data-type="withDesc" @click="onShowShareSheet" />
      <van-share-sheet :show="show.withDesc" title="立即分享给好友" :options="optionsWithDesc" description="描述信息"
