<template>
  <div class="vc-dropdown-wrapper">
    <view :class="['vc-dropdown-menu', menuIndex > -1 ? 'vc-hairline--bottom' : null]" :style="menuStyled">
      <view v-for="(item, index) in menus" :key="index" :class="['vc-dropdown-menu__title', item.active ? 'is-active' : null, item.disabled ? 'is-disabled' : null]" :style="{color: item.active ? activeColor : null, 'z-index': zIndex}" @click="onClickMenu(item, index)">
