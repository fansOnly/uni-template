<template>
  <vc-popup :visible="visible" round position="bottom" :max-height="rowHeight * rows" border :close-on-click-overlay="closeOnClickOverlay" body-style="padding-bottom: 32rpx;" @click-overlay="clickOverlay" @close="close" @after-leave="show = false">
    ...
    <view class="vc-picker-body" :style="{ 'height': rowHeight * rows + 'px' }" @touchmove.stop="noop">
      <vc-picker-column v-for="(item, index) in renderOptions" :key="index" class="vc-picker-col" :options="item.data" :current="getCurrentValue(item)" :rows="rows" :row-height="rowHeight" :style="{'max-width': 'calc(100% / '+renderOptions.length+');'}" @change="onChange($event, index)"></vc-picker-column>
