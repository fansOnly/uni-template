<template>
  <vc-picker :visible="visible" :values="initialValues" :options="[PROVINCE_LIST, CITY_LIST, COUNTY_LIST]" option-key="index" :title="title" :rows="rows" :row-height="rowHeight" cascade :config="[{ column: 0, rollback: true }, { column: 1, rollback: true }]" :close-on-click-overlay="closeOnClickOverlay" @close="close" @update="onUpdate" @confirm="onConfirm"></vc-picker>
</template>
