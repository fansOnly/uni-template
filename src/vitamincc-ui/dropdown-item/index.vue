<template>
  <div v-show="showWrapper" class="vc-dropdown-item-wrapper" :style="wrapperStyled">
    <vc-popup :visible.sync="visible" custom-style="position: absolute;" :duration="duration" :overlay-style="overlayStyled" max-height="40vh" :position="direction === 'down' ? 'top' : 'bottom'" :overlay="overlay" @click-overlay="onClickOverlay" @after-leave="onClosed">
      <vc-cell v-for="(item, index) in options" :key="index" :title="item.text" is-link :disabled="item.disabled" :border="index < options.length - 1" @click="onClickItem(item, index)">
