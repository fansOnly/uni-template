<template>
  <view class="vc-navigation-wrapper">
    <view v-if="usePlaceholder" class="navigation-block" :style="{ 'height': navHeight +'px' }"></view>
    <view v-show="navHeight" class="vc-navigation" :style="headStyled">
      ...
      <view class="vc-navigation-buttons" :style="buttonStyled">
        <slot name="icon">
          <vc-icon v-if="showBack" class="vc-navigation__button--back" name="back" :color="iconColor" size="22" @click="navigateBack" />
          <vc-icon v-if="showHome" class="vc-navigation__button--home" name="home" :color="iconColor" size="22" @click="reLaunchHome" />
        </slot>
      </view>
      <slot>
        <view v-if="title" class="vc-navigation__title vc-ellipsis" :style="titleStyled">{{title}}</view>
      </slot>
    </view>
  </view>
</template>
