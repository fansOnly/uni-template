...
-->
<template>
  <view :class="['vc-upload-wrapper', disabled ? 'vc-upload-wrapper--disabled' : null]">
    <template v-if="fileList.length">
      <view v-for="(item, index) in fileList" :key="index" class="vc-upload__file" :style="style">
        ...
        <!-- 上传状态 -->
        <view v-if="showLoading && loading && index === fileList.length - 1" class="vc-upload__loading">
          <vc-loading type="shine" color="#fff" size="32" vertical><text style="color:#fff;">上传中</text></vc-loading>
        </view>
        <!-- 上传失败状态 -->
        <view v-if="item.status === 'fail'" class="vc-upload__loading">
          <vc-icon name="fail" size="32" color="#fff" />
          <view class="vc-upload__loading-text--fail">上传失败</view>
          <view class="vc-upload__button-retry" @click="reUpload(index)">重新上传</view>
        </view>
        <!-- 上传成功状态 -->
        <view v-if="item.status === 'success'" class="vc-upload__loading" @click="onClickItem(index)">
          <vc-icon name="success" size="32" color="#fff" />
          <view class="vc-upload__loading-text--fail">上传成功</view>
        </view>
      </view>
    </template>
    ...
    <!-- 重新上传 -->
    <vc-action-sheet :visible.sync="visible" :data="options" show-cancel @select="onSelect"></vc-action-sheet>
  </view>
</template>
