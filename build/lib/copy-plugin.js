...
const projectRoot = path.join(cwd, `src/${project}`)
...
copyPluginsList.push({
  ...
  to: path.join(cwd, 'dist', modePath, platformPath, 'static/images')
})
...
if (isCustomTabBar) {
  copyPluginsList.push({
    ...
    to: path.join(cwd, 'dist', modePath, platformPath, 'custom-tab-bar')
  })
}
...
if (fs.existsSync(wxcPath)) {
  copyPluginsList.push({
    from: wxcPath,
    to: path.join(cwd, 'dist', modePath, platformPath, 'wxcomponents')
  })
}
